...
// if (!empty($_REQUEST['tttt'])) {

    //$tttt = mysql_real_escape_string($_REQUEST['tttt']);
    if (isset($_POST['tttt'])){
      $tttt = trim($_POST['tttt']);
    }
   ...
   /***type produit***/
    try{
        $stmt = $db->prepare("select ID_Produit,ID_TypeProduit,Produit_name,Prix,Description,Status FROM produit
                              ...
                              where Produit_name LIKE '%".$tttt."%' or ID_Produit = '".$tttt."'
                              order by ID_Produit");
        $stmt->execute();
